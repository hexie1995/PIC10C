...
public:
	// A vector of Cards
	Hand();
	void dealer_pushback_Card(Card A);
	void pushback_Card(Card A);
	void print_Hand();
	void print_dealer_hand();
	double print_total();
	double print_dealer_total();
	void delete_myhand();
	void output_dealer_hand(ofstream& F);
	void output_hand(ofstream& F);
...
public:
	...
	Player(int m);
	void print_money();
	int money_left() const;
	Hand& myhand();
	void change_money(int bet, bool op);
	void delete_hand();
	// You decide what functions you'll need...
